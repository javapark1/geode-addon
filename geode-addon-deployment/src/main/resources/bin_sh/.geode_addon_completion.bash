...
if [ "$GEODE_ADDON_HOME" != "" ]; then
   SCRIPT_DIR=$GEODE_ADDON_HOME/bin_sh
else
   ...
   *)
      if [ "$second_word" == "cp_sub" ]; then
         if [ $len -gt 3 ]; then
            ...
            if [ $len -lt 4 ]; then
               type_list=`ls $GEODE_ADDON_WORKSPACE/clusters`
            fi
            ...
            if [ $len -lt 4 ]; then
               type_list=`ls $GEODE_ADDON_WORKSPACE/apps`
            fi
      else
...
for i in $commands; do
   if [ "$i" != "setenv.sh" ]; then
      if [ "$i" == "cp_sub" ]; then
         cp_commands=`ls $SCRIPT_DIR/cp_sub`
