...
# Required:
# ---------
# GEODE_ADDON_HOME   The geode-addon root directory path.
# GEODE_HOME         Geode root directory path
#
# Optional:
# ---------
# GEODE_ADDON_WORKSPACE 
#                        User's geode-addon directory where user specifics such as cluster, pods, 
#                        apps and bundles are stored. If not specified then the geode-addon home
#                        directory ($GEODE_ADDON_HOME) is assigned.
...
#                        This will be prepended to the geode-addon class paths.
...
# 
# User geode-addon directory
#
if [ -z $GEODE_ADDON_WORKSPACE ]; then
   export GEODE_ADDON_WORKSPACE=$BASE_DIR
fi

# 
# Geode Enterprise home directory
#
#GEODE_HOME=
...
# Geode clusters and run client programs.
#
...
#
# Set GEODE_ADDON_WORKSPACE if remotely executed
#
if [ "$REMOTE_SPECIFIED" == "true" ] && [ "$WORKSPACE_ARG" != "" ]; then
   export GEODE_ADDON_WORKSPACE=$WORKSPACE_ARG
fi

#
# Source in the workspaces setenv.sh file (mainly for license keys)
#
if [ -f "$GEODE_ADDON_WORKSPACE/../setenv.sh" ]; then
   __SCRIPT_DIR=$SCRIPT_DIR
   __GEODE_ADDON_WORKSPACE=$GEODE_ADDON_WORKSPACE
   . $GEODE_ADDON_WORKSPACE/../setenv.sh
   SCRIPT_DIR=$__SCRIPT_DIR
   export GEODE_ADDON_WORKSPACE=$__GEODE_ADDON_WORKSPACE
fi

#
# Source in the workspace setenv.sh file that contains user configured variables
#
if [ -d $GEODE_ADDON_WORKSPACE ] && [ -f $GEODE_ADDON_WORKSPACE/setenv.sh ] && [ "$GEODE_ADDON_WORKSPACE" != "$BASE_DIR" ]; then
...
# path is "<geode-addon-dir>/products". This path is mounted as "~/products" in the
# guest OS.
#
DEFAULT_HOST_PRODUCTS_DIR="$GEODE_ADDON_WORKSPACE/products"
...
if [ -z $PODS_DIR ]; then
   PODS_DIR=$GEODE_ADDON_WORKSPACE/pods
fi
if [ -z $POD_DIR ]; then
...
# Remote Bundle URLs
#
BUNDLE_REPO=https://github.com/javapark1/geode-addon-bundles
BUNDLE_LIST_URL=https://github.com/javapark1/geode-addon-bundles/tree/master/bundles
BUNDLE_DIR_URL=https://github.com/javapark1/geode-addon-bundles/raw/master/bundles
...
if [ "$GEODE_ADDON_WORKSPACE" != "" ] && [ "$GEODE_ADDON_WORKSPACE" != "$BASE_DIR" ]; then
   __CLASSPATH="$__CLASSPATH:$GEODE_ADDON_WORKSPACE/plugins/*:$GEODE_ADDON_WORKSPACE/lib/*"
fi
__CLASSPATH="$__CLASSPATH:$BASE_DIR/plugins/*:$BASE_DIR/lib/*"
__VERSION_DIR=v${GEODE_VERSION:0:1}
